...
healthSearch.prototype.disease = function(disease) {
  let promise = new Promise(function(resolve, reject) {
    ...
    let url = `https://api.betterdoctor.com/2016-03-01/doctors?query=${disease}&location=47.6080%2C-122.3351%2C20&user_location=47.6062%2C-122.3321&sort=best-match-asc&skip=0&limit=4&user_key=${apiKey}`;
...
healthSearch.prototype.doctorName = function(name) {
  let promise = new Promise(function(resolve, reject) {
    let request = new XMLHttpRequest();
    let url = `https://api.betterdoctor.com/2016-03-01/doctors?name=${name}&location=47.6080%2C-122.3351%2C20&user_location=47.6062%2C-122.3321&sort=best-match-asc&skip=0&limit=4&user_key=${apiKey}`;
  ...
  promise.then(function(response) {
    ...
    if (response.length === 0){
      ...
      for(let i = 0; i < response.length; i++) {
        ...
        $('#results').append(`<li>Address: ${response[i].practices[0].visit_address.street} <br> ${response[i].practices[0].visit_address.city}, ${response[i].practices[0].visit_address.state}, ${response[i].practices[0].visit_address.zip}</li>`);
        $('#results').append(`<li>Phone: ${response[i].practices[0].phones[0].number}</li>`);
        $('#results').append(`<li>Languages: ${response[i].practices[0].languages[0].name}</li>`);
        if(response[i].practices[0].website !== undefined) {
